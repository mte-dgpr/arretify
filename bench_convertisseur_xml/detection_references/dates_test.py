...
class TestParseDateFunction(unittest.TestCase):
    ...
    def test_date2_valid_cases(self):
        ...
        assert handle_date_match_groupdict(_run_search(DATE2_RES, "03/03/99")) == date(1999, 3, 3)
        assert handle_date_match_groupdict(_run_search(DATE2_RES, "10/10/2000")) == date(2000, 10, 10)
        assert handle_date_match_groupdict(_run_search(DATE2_RES, "01/12/1999")) == date(1999, 12, 1)
        assert handle_date_match_groupdict(_run_search(DATE2_RES, "31/01/1990")) == date(1990, 1, 31)

    def test_date1_invalid_cases(self):
        # Test invalid "DATE1_RES" cases
        assert _run_search(DATE1_RES, "janvier 2023") is None # Missing day
        assert _run_search(DATE1_RES, "1 janvier") is None # Missing year
        assert _run_search(DATE1_RES, "1er unknownmonth 2023") is None # Invalid month
        assert _run_search(DATE1_RES, "1er janvier 23a") is None # Invalid year format

    def test_date2_invalid_cases(self):
        # Test invalid "DATE2_RES" cases
        assert _run_search(DATE1_RES, "15/02") is None # Missing year
        assert _run_search(DATE1_RES, "15/02/20a3") is None # Invalid year format
        assert _run_search(DATE1_RES, "15-02-2023") is None # Wrong delimiter
        assert _run_search(DATE1_RES, "15//2023") is None # Missing month

    def test_ambiguous_or_malformed_inputs(self):
        # Test ambiguous or malformed inputs
        assert _run_search(DATE1_RES, "2023 janvier 1er") is None # Wrong order
        assert _run_search(DATE2_RES, "15/02/99/extra") is None # Extra text

    def test_edge_cases(self):
        # Test edge cases
        assert handle_date_match_groupdict(_run_search(DATE1_RES, "1er janvier 00")) == date(2000, 1, 1)  # Year 200
        assert handle_date_match_groupdict(_run_search(DATE1_RES, "1 janvier 99")) == date(1999, 1, 1)  # Year 199
        assert _run_search(DATE1_RES, "") is None # Empty string
