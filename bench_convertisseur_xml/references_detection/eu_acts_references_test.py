...
class TestParseEuActsReferences(unittest.TestCase):
    ...
    def test_domain_before(self):
        assert _parsed_elements('Bla bla de la directive (CE) n° 1013/2006 du 22 juin 2006') == [
            'Bla bla de la ',
            '<a class="dsr-document_reference" data-uri="eu://directive_1013%2F2006_CE">directive (CE) n° 1013/2006</a>',
            ' du 22 juin 2006',
        ]
    ...
    def test_domain_after(self):
        assert _parsed_elements('VU la directive 2010/75/UE du 24 novembre 2010') == [
            'VU la ',
            '<a class="dsr-document_reference" data-uri="eu://directive_2010%2F75_UE">directive 2010/75/UE</a>',
            ' du 24 novembre 2010',
        ]
    ...
    def test_with_word_europeen(self):
        assert _parsed_elements('VU le règlement européen (CE) n° 1013/2006 du 22 juin 2006') == [
            'VU le ',
            '<a class="dsr-document_reference" data-uri="eu://r%C3%A8glement_1013%2F2006_CE">règlement européen (CE) n° 1013/2006</a>',
            ' du 22 juin 2006',
        ]
